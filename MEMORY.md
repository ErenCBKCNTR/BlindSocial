...
- Kotlin tabanlı Native MediaProjection Foreground Service entegrasyonu: Android 14+ cihazlarda MediaProjection (Ekran paylaşımı) başlatılırken `flutter_webrtc` veya `flutter_background` gibi paketlerin yarattığı SecurityException (Native App Crash) çökmesini engellemek için, tamamen projeye özel ve saf Kotlin diliyle yazılmış kalıcı (ongoing) bildirim servisi (`NativeScreenShareService`) oluşturuldu. Flutter içerisinden `MethodChannel` (`com.blindsocial.app/foreground`) aracılığıyla LiveKit yayınından hemen önce tetiklenerek güvenli bir yayın ortamı sağlandı.
- Kotlin Foreground Service başlatma ve izin (permission) fixi: Saf Kotlin servisi (NativeScreenShareService) başlatılırken işletim sisteminin anında çökmesine neden olan izin eksiklikleri giderildi. `AndroidManifest.xml` içerisindeki izinler teyit edilerek Dart tarafında `startNativeService` tetiklenmeden önce `POST_NOTIFICATIONS` izninin çalışma anında (runtime) kullanıcıdan zorunlu olarak alınması (`permission_handler` ile) sağlandı. Ayrıca Kotlin tarafındaki NotificationChannel oluşturma kontrolleri Android 8.0 mimarisine uygun olarak teyit edildi.
- Ekran paylaşımı altyapısının kaldırılması ve mikrofon butonu erişilebilirlik (semanticLabel) düzeltmesi: Android 14 cihazlarda yaşatılan kronik "App keeps stopping" sorununu tamamen bertaraf etmek amacıyla, Native ekran paylaşımı/sistem sesi servisleri ve arkasındaki MethodChannel iletişimleri (`MainActivity.kt`, `NativeScreenShareService.kt`, `AndroidManifest.xml` ve `chat_screen.dart` vb.) kod tabanından tamamen silindi. Ek olarak, Mikrofon Ayarları butonundaki TalkBack "etiketsiz okuma" hatası `Tooltip` yerine `Semantics` widget'ı konularak erişilebilir hale getirildi.
- Radyo Tiyatrosu Google Drive bağlantı çözümü ve uygulama geneli TalkBack çift okuma (etiketsiz node) fixi: Radyo Tiyatrosu oynatıcısında Google Drive API değişikliklerinden kaynaklanan "Herkese Açık Değil" sabit hatası dinamik hata gösterimi ve gelişmiş URL yönlendirmesi ile çözüldü. Arayüzdeki (özellikle Sohbet Ekranındaki) dairesel düğmelerin `IconButton` kaynaklı çift (veya etiketsiz) okumasını önlemek için evrensel bir `AccessibleIconButton` oluşturuldu ve tüm ilgili butonlara uyarlandı.
- Meydan Ekranı Gönderi Silme ve Scroll/Semantics Optimizasyonu: Meydan (Square Screen) sayfasındaki gönderi listesinin son elemanlara ulaşılamaması (RefreshIndicator ile çakışması) sorunu `ListView` padding değeri artırılarak (bottom: 250) çözüldü. Gönderi ve yorum silme işlemlerinde görsel `SnackBar` onayları eklendi ve silme butonu yetkileri (Sadece sahibi ve adminler) kısıtlandı. Ekran genelindeki (Beğen, Yorum, Sil, Düzenle, Şikayet Et) tüm ikon butonları `Tooltip` yerine `Semantics(button: true) + ExcludeSemantics` formuna güncellenerek ekran okuyucunun (TalkBack) "etiketsiz düğme" şeklinde çift okuma hatası tamamen giderildi.
- Sesli Odalar LiveKit Sunucu Taşıması: Eski test sunucusu devreden çıkartılarak Hetzner üzerindeki prodüksiyon sunucusuna (`wss://live.cabukcan.com`) geçiş yapıldı. İstemci tarafında `chat_screen.dart` güncellenirken sunucu tarafında (`functions/src/index.ts`) güvenlik amacıyla LiveKit token üretiminde API bilgilerinin sadece `process.env` (ortam değişkenleri) üzerinden okunması sıkılaştırıldı. Gizli API anahtarları asla kod tabanına gömülmedi. (Not: P2P arama mantığını içeren `bs_bib_call_screen.dart` bu değişikliğin dışında tutuldu).
